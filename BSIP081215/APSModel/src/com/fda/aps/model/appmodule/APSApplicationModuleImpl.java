...
import com.fda.aps.model.viewobject.APSSearchViewImpl;
import com.fda.aps.model.viewobject.AapFundingObjectViewImpl;
...
import com.fda.aps.model.viewobject.DetailedExpensesViewImpl;
import com.fda.aps.model.viewobject.DetailedExpensesViewRowImpl;
...
public class APSApplicationModuleImpl extends ApplicationModuleImpl implements APSApplicationModule {
    ...
    public void createNewRequest(String centerCode, String actionType,
        ...
        String centerSequence =
            generateCenterLineSequence(centerCode).toString();
        if (centerSequence.length() == 1) {
            centerSequence = "000" + centerSequence;
        } else if (centerSequence.length() == 2) {
            centerSequence = "00" + centerSequence;
        } else if (centerSequence.length() == 3) {
            centerSequence = "0" + centerSequence;
        }
    ...
    public void executeSubmittedRequest(String requestId) {
        ...
        Row row = submittedRequestsView.first();

        RequestsViewRowImpl requestsViewRowImpl = (RequestsViewRowImpl)row;

        if (requestsViewRowImpl != null) {
            RowIterator rowIterator =
                requestsViewRowImpl.getPointOfContactView();
    ...
    public void approveAcquisitionRequest(String centerItemNo,
        ...
        Row requestRow = getRequestsView().findByKey(reqIdKey, 1)[0];
        requestRow.setAttribute("SubmitFlag", "S");
        ...
        List<String> collabEmailList = getCollaborationEmail(requestId);
        if (collabEmailList.size() > 0) {
            for (String collab : collabEmailList) {
    ...
    public void approveBudgetRequest(String centerItemNo, String requestId) {
        ...
        ADFContext ctx = ADFContext.getCurrent();
        ViewObjectImpl aapNotificationView = getMassApprovalQuery();
        ViewCriteria criteria =
            aapNotificationView.getViewCriteria("byRequestId");
    ...
    public void rejectAcquisitionRequest(String centerItemNo,
        ...
        for (String role : epRoles) {
            if (role.contains("ACQ_USER") || role.contains("ACQ_APRV")) {
    ...
    public void refreshDashboard(String user) {
        ...
        ViewCriteria completeCriteria =
            completedStatusView.getViewCriteria("CompletedStatus");
        completedStatusView.applyViewCriteria(completeCriteria);
        completedStatusView.setNamedWhereClauseParam("bind_Center",
                                                     AAPModelUtils.deriveCenterFromRole());
        completedStatusView.setNamedWhereClauseParam("bind_User",
                                                     secContext.getUserName());
        ...
        ViewCriteria byQueryBySuperUser =
            notificationView.getViewCriteria("QueryBySuperUser");
            ...
            if (secContext.isUserInRole("AAPACQNAPPROVE")) {
                ...
                if (secContext.isUserInRole("AAPBUDGETCREATE")) {
                    ...
                    if (secContext.isUserInRole("AAPBUDGETAPPROVE")) {
                        ...
                        ViewCriteriaRow collaborationCriteriaRow =
                            createCollaborationCriteria(byQueryBySuperUser);


                        if (collaborationCriteriaRow != null) {

                            byQueryBySuperUser.addRow(collaborationCriteriaRow);

                        }
                        notificationView.applyViewCriteria(byQueryBySuperUser);
                    ...
                    ViewCriteriaRow collaborationCriteriaRow =
                        createCollaborationCriteria(byAcqnUserLeadandBudgetUser);


                    if (collaborationCriteriaRow != null) {

                        byAcqnUserLeadandBudgetUser.addRow(collaborationCriteriaRow);
                ...
                if (secContext.isUserInRole("AAPBUDGETAPPROVE")) {
                    ViewCriteriaRow collaborationCriteriaRow =
                        createCollaborationCriteria(byAcqnBudgetUserandLead);
                    ...
                    if (collaborationCriteriaRow != null) {
                        //notificationView.applyViewCriteria(collaborationCriteria, true);
...
*/
    ...
    public void populateDueDate(String actionType) {

        ViewObject slaCuoffDateVO = getSlaCutoffdateCalcVO();
        Number slaDays = null;
        Date cutOffDate = null;

        RowIterator rowIter =
            slaCuoffDateVO.findByAltKey("keyByReference", new Key(new Object[] { actionType }),
                                        -1, Boolean.TRUE);
        if (rowIter.hasNext()) {
            Row referenceRow = rowIter.next();
            slaDays = (Number)referenceRow.getAttribute("SlaDays");
            cutOffDate = (Date)referenceRow.getAttribute("CutOffDate");
        ...
        Row currentRequestRow = requestView.getCurrentRow();
        if (slaDays != null && cutOffDate != null) {
            ...
            if (awardDate != null) {
                ...
                try {
                    Date newDate =
                        dateBeforeNDays(awardDate, slaDays.intValue());
                    if (newDate.compareTo(cutOffDate) == -1) {
                        currentRequestRow.setAttribute("DueDate", newDate);
                    } else {
                        currentRequestRow.setAttribute("DueDate", cutOffDate);
                    }
                } catch (Exception ex) {
    ...
    public void cloneRequest(String requestId) {
        ...
        String centerCode = AAPModelUtils.getCenterFromRole();
        Key reqIdKey = new Key(new Object[] { requestId });
        ViewObject requestVO = getRequestsView();

        //SequenceImpl seq = generateCenterLineSequence(centerCode);
        SequenceImpl seq = new SequenceImpl("REQUEST_SEQ", getDBTransaction());
        Number reqNo = seq.getSequenceNumber();
        String centerSequence =
            generateCenterLineSequence(centerCode).toString();
        ...
        String newCenterLine =
            originalCenterItemNo.substring(0, dashLastIndex) + "-" +
            centerSequence;
        ;
    ...
    public void addSLA(String description, String slaDays,
                       oracle.jbo.domain.Date cutoffDate, String type) {
        ...
        newSlaRow.setAttribute("Type", type);
    ...
    private List<String> getCollaborationEmail(String requestId) {
        ViewObjectImpl collaborationEmailVO = getAapCollaborationEmailVO();
        ViewCriteria byRequestId =
            collaborationEmailVO.getViewCriteria("byRequestId");
        collaborationEmailVO.setNamedWhereClauseParam("bind_RequestId",
                                                      requestId);
        collaborationEmailVO.applyViewCriteria(byRequestId);
        collaborationEmailVO.executeQuery();
        List<String> collabEmail = new ArrayList<String>();

        while (collaborationEmailVO.hasNext()) {
            Row row = collaborationEmailVO.next();
            if (row != null) {
                String center =
                    (String)row.getAttribute("ReferenceDescription");
                collabEmail.add(center + "-BDGT_USER@fda.hhs.gov");
            }
        }
        return collabEmail;
    }
    ...
    private ViewCriteriaRow createCollaborationCriteria(ViewCriteria collabViewCrietria) {

        //ViewCriteria collabViewCrietria = null;
        ...
        searchCollabCenter.setNamedWhereClauseParam("bind_Center",
                                                    AAPModelUtils.getCenterFromRole());
        ...
        if (requests.size() > 0) {
            //collabViewCrietria = viewObject.createViewCriteria();
            collabViewCrietria.setName("CollaborationCriteria");
            collabViewCrietria.setConjunction(ViewCriteria.VC_CONJ_OR);
            criteriaRow = collabViewCrietria.createViewCriteriaRow();
            ...
            //criteriaRow = collabViewCrietria.createViewCriteriaRow();
            ViewCriteriaItem criteriaRowItem1 =
                criteriaRow.ensureCriteriaItem("WfStatus");
            criteriaRowItem1.setOperator(JboCompOper.OPER_IN);
            criteriaRowItem1.setValueMaxCardinality(2);
            criteriaRowItem1.setValueMinCardinality(2);
            criteriaRowItem1.setValue(0, "ACQUSITION_APPROVED");
            criteriaRowItem1.setValue(1, "BUDGET_USER_SUBMITTED");


        }

        //collabViewCrietria.addRow(criteriaRow);
        return criteriaRow;
    }
    ...
    private ViewCriteriaRow createBudgetWFStatusCrieriaRow(ViewCriteria criteria) {
        ViewCriteriaRow criteriaRow = null;
        criteriaRow = criteria.createViewCriteriaRow();
        ViewCriteriaItem criteriaRowItem =
            criteriaRow.ensureCriteriaItem("WfStatus");
        criteriaRowItem.setOperator(JboCompOper.OPER_IN);
        criteriaRowItem.setValueMaxCardinality(2);
        criteriaRowItem.setValueMinCardinality(2);
        criteriaRowItem.setValue(0, "ACQUSITION_APPROVED");
        criteriaRowItem.setValue(1, "BUDGET_USER_SUBMITTED");
        return criteriaRow;
    }


    /**
     * Container's getter for CompetitionTypeVO.
     * @return CompetitionTypeVO
     */
    public ApsReferenceViewImpl getCompetitionTypeVO() {
        return (ApsReferenceViewImpl)findViewObject("CompetitionTypeVO");
    }

    /**
     * Container's getter for AapFundingObjectView.
     * @return AapFundingObjectView
     */
    public ViewObjectImpl getAapFundingObjectView() {
        return (ViewObjectImpl)findViewObject("AapFundingObjectView");
    }


    /**
     * Container's getter for SlaCutoffdateCalcVO.
     * @return SlaCutoffdateCalcVO
     */
    public AapSlaCutoffdateVOImpl getSlaCutoffdateCalcVO() {
        return (AapSlaCutoffdateVOImpl)findViewObject("SlaCutoffdateCalcVO");
    }

    public void setBindTypeToSlaCutoffDate(String actionType) {

        ViewObjectImpl vo = getAapFundingObjectView();
        ViewCriteria vc = vo.getViewCriteria("ByType");
        vo.applyViewCriteria(vc);
        vo.setNamedWhereClauseParam("bind_Type", actionType);
        vo.executeQuery();
    }

    /**
     * Container's getter for StartDateSetupVO.
     * @return StartDateSetupVO
     */
    public ApsReferenceViewImpl getStartDateSetupVO() {
        return (ApsReferenceViewImpl)findViewObject("StartDateSetupVO");
    }

    public void executeFundingObjectAccessor(String type) {
        ViewObjectImpl detailView = getDetailedExpensesView();
        DetailedExpensesViewRowImpl row =
            (DetailedExpensesViewRowImpl)detailView.getCurrentRow();
        RowSet rowset = row.getAapFundingObjectView();
        rowset.setNamedWhereClauseParam("bind_Type", type);
        rowset.executeQuery();
    }

    /**
     * Container's getter for EffectiveEndDateVO.
     * @return EffectiveEndDateVO
     */
    public ApsReferenceViewImpl getEffectiveEndDateVO() {
        return (ApsReferenceViewImpl)findViewObject("EffectiveEndDateVO");
    }

    private Number generateCenterLineSequence(String centerCode) {

        SequenceImpl seq =
            new SequenceImpl(centerCode + "_REQ_SEQ", getDBTransaction());
        Number centerLineSeq = seq.getSequenceNumber();

        return centerLineSeq;
    }

    public void refreshFundingObject(String type) {
        ViewObjectImpl detailView = getDetailedExpensesView();
        //Row[] rows = detailView.getAllRowsInRange();
        while (detailView.hasNext()) {
            Row row = detailView.next();
            DetailedExpensesViewRowImpl detailExpensesRow =
                (DetailedExpensesViewRowImpl)row;
            RowSet rowset = detailExpensesRow.getAapFundingObjectView();
            rowset.setNamedWhereClauseParam("bind_Type", type);
            rowset.executeQuery();
        }
    ...
    /**
     * Container's getter for SearchByControlKeyVO.
     * @return SearchByControlKeyVO
     */
    public RequestsViewImpl getSearchByControlKeyVO() {
        return (RequestsViewImpl)findViewObject("SearchByControlKeyVO");
    }

    /**
     * Container's getter for AapCollaborationEmailVO.
     * @return AapCollaborationEmailVO
     */
    public ViewObjectImpl getAapCollaborationEmailVO() {
        return (ViewObjectImpl)findViewObject("AapCollaborationEmailVO");
    }

    /**
     * Container's getter for ReferenceByIdView.
     * @return ReferenceByIdView
     */
    public ApsReferenceViewImpl getReferenceByIdView() {
        return (ApsReferenceViewImpl)findViewObject("ReferenceByIdView");
    }
}
